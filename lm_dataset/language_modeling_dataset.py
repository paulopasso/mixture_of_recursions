...
class LanguageModelingDataset(IterableDataset):
    def __init__(self, dataset, tokenizer: PreTrainedTokenizer, max_length, continuous=True, 
                 ...
                 local_shuffling=True, add_bos_token=False, max_tokens: int | None = None, max_batches: int | None = None,):
        ...
        # Optional budgets for fast debugging
        self.max_tokens_budget = max_tokens
        self.max_batches_budget = max_batches
    ...
    def __iter__(self) -> Iterator[dict[str, torch.Tensor]]:       
        ...
        # Helper to check budgets
        def budget_exhausted(tokens_to_emit: int = 0) -> bool:
            if self.max_tokens_budget is not None and (emitted_tokens + tokens_to_emit) > self.max_tokens_budget:
                return True
            if self.max_batches_budget is not None and emitted_batches >= self.max_batches_budget:
                return True
            return False
        ...
        while data_remaining:
            ...
            for i, input_ids in enumerate(self.full_samples):
                ...
                if budget_exhausted(tokens_to_emit=len(input_ids)):
                    data_remaining = False
                    break
                ...
                if self.transforms is not None:
                    for transform in self.transforms:
                        final_sample = transform(final_sample)
                emitted_tokens += int(len(input_ids))
                emitted_batches += 1
                yield final_sample
